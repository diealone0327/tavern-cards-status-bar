...
const props = defineProps<{
  protagonist?: { 道具?: string[]; 丹药?: string[]; 法宝?: string[]; 武器?: string[]; };
}>();
...
const itemDesc: Record<string, string> = {
  ...
  '破界丹': '直接突破当前大境界（无视天劫）',
  // 法宝
  '万象储物戒': '内含百里洞天，可存放活物种植灵药',
  '太虚幻神面': '随意变换容貌身形气息修为模拟',
  '不灭灵光罩': '自动护体灵光，可挡合体期全力一击',
  '周天聚灵阵盘': '形成巨型聚灵阵，灵气浓度提升5倍',
  '缚天神索': '束缚大乘期以下任何目标',
  '九幽镇魂钟': '一响镇魂三响灭魂，克制邪祟',
  '乾坤遁虚符': '传送至万里内任意去处，无视禁制结界',
  '通天神火炉': '熔炼万物提升丹药法器品质，可炼仙品',
  // 武器
  '裂天剑': '古宝，一剑风云变色，斩破同阶防御',
  '苍穹破星弓': '无箭以灵力凝矢，百里取人性命',
  '九幽冥王枪': '枪出如龙携带冥界死气',
  '不灭薪火扇': '一扇生风二扇燃火三扇焚天',
  '玄黄镇岳印': '如山岳压顶镇压一方天地',
  '太阴月轮': '弯月飞刃无声无息月夜威力翻倍',
  '渡厄金锏': '专破护体罡气阵法结界，克制魔修',
  '轮回剑': '斩杀者神魂俱灭，亦可施展轮回剑意',
};
