export const Schema = z.object({
  ...
  user: z.object({
    ...
    宗门职位: z.enum(['杂役弟子','外门弟子','内门弟子','真传弟子','长老']).prefault('杂役弟子'),
    持有物品: z.record(z.coerce.number().transform(v => Math.max(0, v))).prefault({}),
    物品分类: z.record(z.string()).prefault({}),
    储物袋: z.record(z.coerce.number().transform(v => Math.max(0, v))).prefault({}),
    储物袋分类: z.record(z.string()).prefault({}),
    仓库: z.record(z.coerce.number().transform(v => Math.max(0, v))).prefault({}),
    仓库分类: z.record(z.string()).prefault({}),
  ...
  聚宝盆: z.object({
    ...
    ...data,
    冷却状态: data.盆内物品 || data.剩余时间 ? data.冷却状态 : true,
  })),
  宗门: z.object({
  ...
  秘境: z.object({
    ...
    秘境列表: z.record(z.string()).prefault({}),
  }).prefault({}),
